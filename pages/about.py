...
def team_section():
    ...
    <div style="text-align: center; margin-bottom: 3rem; padding: 2rem 0;">
        <h2 style="font-size: 2.5rem; font-weight: 700; color: #1f2937; margin-bottom: 1rem; 
                   background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                   -webkit-background-clip: text; -webkit-text-fill-color: transparent; 
                   background-clip: text; text-align: center;">Meet Our Team</h2>
        <p style="font-size: 1.2rem; color: #6b7280; max-width: 600px; margin: 0 auto; 
                  line-height: 1.6; text-align: center;">
            Passionate experts dedicated to making public health data accessible and actionable for everyone
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    for i, member in enumerate(team_members):
        with cols[i % 2]:
            ...
            st.markdown(f"""
            <div style="background: white; padding: 2.5rem 2rem; border-radius: 20px; text-align: center; 
                        box-shadow: 0 10px 30px rgba(0,0,0,0.08); transition: all 0.3s ease; 
                        height: 420px; display: flex; flex-direction: column; align-items: center; 
                        justify-content: flex-start; margin-bottom: 2.5rem; border: 1px solid #f3f4f6; 
                ...
                <img src="data:image/jpeg;base64,{get_image_base64(member['image'])}" 
                     style="width: 130px; height: 130px; border-radius: 50%; object-fit: cover; 
                            border: 5px solid #e5e7eb; box-shadow: 0 6px 20px rgba(0,0,0,0.1); 
                ...
                <p style="color: #3b82f6; font-weight: 600; margin-bottom: 1.5rem; font-size: 1rem; 
                          text-align: center; text-transform: uppercase; letter-spacing: 1px; 
                          flex-shrink: 0; width: 100%; padding: 0.5rem 0; 
                          background: rgba(59, 130, 246, 0.1); border-radius: 20px;">{member['role']}</p>
                <p style="color: #6b7280; font-size: 0.95rem; line-height: 1.7; text-align: center; 
                          margin: 0; flex-grow: 1; display: flex; align-items: flex-start; width: 100%; 
                          padding: 0; overflow-wrap: break-word; word-wrap: break-word; 
                          justify-content: center;">{member['description']}</p>
            </div>
            """, unsafe_allow_html=True)
